    ...
    for index, value in enumerate(character.all_classes_and_jobs):
        ...
        for idx,classes in enumerate(value):
            ...
            for each in classes:
                ...
                label1 = tk.Canvas(master, width = 40, height=40, highlightthickness = 0)
                ...
                text = tk.Canvas(master,width = 30, height=30, highlightthickness = 0)
                hwnd = text.winfo_id()
                colorkey = win32api.RGB(0,0,0)
                wnd_exstyle = win32gui.GetWindowLong(hwnd, win32con.GWL_EXSTYLE)
                new_exstyle = wnd_exstyle | win32con.WS_EX_LAYERED
                ...
                text.create_text(15,15,text = f"{character.level_dict[each]:02}", font=("Courier New", 18), fill="YELLOW")
...
def main():
    ...
    button1 = tk.Button(master, text="Name", bg=DARK_BLUE, fg="YELLOW", command=lambda:pull_data_name(mommy, e1.get()))
    button1.grid(row=0, column=5)
    button2 = tk.Button(master, text="ID", bg=DARK_BLUE, fg="YELLOW", command=lambda:pull_data_id(mommy, e1.get()))
    button2.grid(row=0, column=6)
    display_button = tk.Button(master, text = "Display", bg=DARK_BLUE,fg="YELLOW", command = lambda:display_info(mommy, master))
